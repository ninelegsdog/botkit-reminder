...
def test_setup_tracing_called(mock_setup: MagicMock) -> None:
    mock_tracer = MagicMock()
    mock_setup.return_value = mock_tracer

    from src.core.tracing import setup_tracing

    tracer = setup_tracing(service_name="reminder", otlp_endpoint="http://localhost:4318/v1/traces")

    mock_setup.assert_called_once_with(service_name="reminder", otlp_endpoint="http://localhost:4318/v1/traces")
...
async def test_tracing_middleware_creates_span() -> None:
    with patch("botkit_core.tracing.trace.get_tracer") as mock_get_tracer:
        ...
        async def handler(ev: Any, dt: dict[str, Any]) -> str:
            return "result"

        result = await mw(handler, cast(Any, event), data)
