...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(text, reply_markup=reply_menu("⏰ Мои напоминания", "📣 Рассылки", "➕ Добавить"))
...
async def time_entered(message: Message, state: FSMContext) -> None:
    ...
    async with get_session() as session:
        from src.core.bot_factory import state as bot_state
        service = ReminderService(session, bot_state.bot)
    ...
    await state.clear()
    await message.answer(f"✅ Напоминание создано! ID: {reminder.id}", reply_markup=reply_menu("⏰ Мои напоминания", "📣 Рассылки"))
...
async def my_reminders(message: Message) -> None:
    ...
    if not reminders:
        await message.answer("Нет напоминаний", reply_markup=reply_menu("➕ Добавить", "📣 Рассылки"))
        return
    lines = []
    for r in reminders:
        when = r.fire_at.strftime("%d.%m.%Y %H:%M") if r.fire_at else r.cron_day or "?"
        lines.append(f"#{r.id} {r.type.value} {when} — {escape(r.text)}")
    await message.answer("\n".join(lines), reply_markup=reply_menu("➕ Добавить", "📣 Рассылки"))
...
async def broadcast_text(message: Message, state: FSMContext) -> None:
    async with get_session() as session:
        from src.core.bot_factory import state as bot_state
        service = BroadcastService(session, bot_state.bot)
        broadcast = await service.create_broadcast(message.text, segment="active")
        await service.send_broadcast(broadcast.id)
    await state.clear()
    await message.answer(f"✅ Рассылка отправлена! ID: {broadcast.id}", reply_markup=reply_menu("⏰ Мои напоминания", "📣 Рассылки"))
...
async def admin_password(message: Message, state: FSMContext) -> None:
    ...
    if verify_password(message.text):
        ...
        await state.clear()
        await message.answer("✅ Админ-доступ открыт", reply_markup=reply_menu("📊 Статистика", "📣 Рассылка", "👥 Подписчики"))
    else:
...
async def unsubscribe(message: Message) -> None:
    ...
    await message.answer("✅ Вы отписались от рассылок", reply_markup=reply_menu("⏰ Мои напоминания", "📣 Рассылки"))
